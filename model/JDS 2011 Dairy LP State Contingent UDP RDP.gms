...
* This is the Australian dairy conference paper model (same as RSMG working paper) but with these modifications:
*1 protein model used in JDS 2007 article Table RDP and relevant equations
*2 updated data tables (3 years of data instead of orginal 2) Table GF
*3 Water use decomposed by season - Table WI?
*4 ??
...
** use with Set ax xMultiSolves / Dummy1, Dummy2 / above;
*$ontext
Table xMultiSolvesTable(ax,axData)      xData table for MultiSolves
$ondelim
$include xMultiSolvesDummy.csv                                                    #New for state contingent model
$offdelim
...
*                 vNumberCows(k,c)         Number of cows in season(s)       (cows)
                 vMilkProd(k,c,s,q)       Production in each month of lact  (L)               ##Disappears with Fixed cow production activities
                 ...
                 vMEintake(k,s,c)           ME intake per day for cows        (MJ)         #for prot model
                 vRDPTransfer(k,s,c)        RDP transfer to ADPLS via rumen   (kg)         #for prot model
                 vDMIntake(k,s,c)           Actual DM intake per day for cows (kg)         #for prot model
...
*                 vNumberCows(k,c)
                 ...
                 vFertReqt(k,e)
                 vWaterReqt(k,s)
                 vMEintake(k,c,s)                                                    #for prot model
                 vRDPTransfer(k,s,c)                                                 #for prot model
                 vDMIntake(k,s,c)                                                    #for prot model
                 vTCForageEstab(k,s)
                 vTCSupplPurch(k,s)
                 vTCCowCosts(k,s)
                 vTRMilkSales(k,s)
...
*           eObjFunctionk(k,s)        Ob max final cash balance       # Equation 1 (part of) Complex cash transfer with overdraft limit
           eObjFunctionk(k)        Ob max final cash balance       # Equation 1 (part of) Basic cash transfer
...
*           ePsupply(k,s,c)          Protein Supply                                    # Equation 10
           eSumMEIntake(k,s,c)      Sums ME intake for RDP requirement                # Equation 10a
           eRDPBalance(k,s,c)       RDP requirements                                  # Equation 10b
           eRDPTransfer(k,s,c)      RDP Protein Supply                                # Equation 10c
           eActualDMIntake(k,s,c)   Sums DM intake for UDP faecal reqt                # Equation 10d
           eADPLSBalance(k,s,c)     UDP Protein Supply                                # Equation 10e
...
***Equation 1 (a part)
eObjFunction..
Sum(k, State(k,'p')* vObjectivek(k) )  =g= vObjective;                                # was equal to

*eObjFunctionk(k,s)..
* Complex tranfer system with overdraft limit
*  +  (vCashTransfer(k,s)$(ord(s)=card(s)) - ODInt*vOverdraft(k,s)$(ord(s)=card(s))
*  - OpCash$(ord(s)=card(s)))
*                               =e= vObjectivek(k)$(ord(s)=card(s));
*Basic cash transfer system
eObjFunctionk(k)..
  +  sum(s, vCashTransfer(k,s) )  =g= vObjectivek(k);                                 # was equal to
...
eRDPBalance(k,s,c)..
  - sum(g, sum(r, sum(w, RDPS(w,g) )      /1000 * vGrainToCows(k,s,c,g,r) ))
  -               sum(f, RDPF(s,f)        /1000 * vForageToCows(k,s,c,f)  )
*  - sum(o, sum(h, sum(f, PFH(o,f)       /1000 * vConsToCows(o,s,c,f,h)  )))
*assume same digestibility as source forage
  -        sum(h, sum(f, RDPH(h,f)        /1000 * vConsToCows(k,s,c,f,h)    ))
  +      RDPperME / 1000 * vMEintake(k,c,s)
*  - vRDProtSlack(s,c)                                                                #This tests what happens if Protein was not a constraint
                                                         =l= 0;                      # less than should work
...
***Equation 10d***********************************
eActualDMIntake(k,s,c)..
 + sum(g, sum(r, vGrainToCows(k,s,c,g,r)   ))
 +        sum(f, vForageToCows(k,s,c,f)    )
*  - sum(o, sum(h, sum(f, vConsToCows(o,s,c,f,h) )))
 + sum(f, sum(h, vConsToCows(k,s,c,f,h)    ))
 - vDMIntake(k,s,c)
                                                        =e= 0;                         # was equal to

***Equation 10e***********************************
eADPLSBalance(k,s,c)..
  - sum(g, sum(r, sum(w, UDPS(w,g) )      /1000 * vGrainToCows(k,s,c,g,r) * UDPtoADPLS))
  -               sum(f, UDPF(s,f)        /1000 * vForageToCows(k,s,c,f)  * UDPtoADPLS)
*  - sum(o, sum(h, sum(f, PFH(o,f)       /1000 * vConsToCows(o,s,c,f,h) * UDPtoADPLS)))
*assume same digestibility as source forage
  -        sum(h, sum(f, UDPH(h,f)        /1000 * vConsToCows(k,s,c,f,h)  * UDPtoADPLS))
  - vRDPTransfer(k,s,c) * RDPtoADPLS
*  - vUDProtSlack(s,c)                                                                #This tests what happens if Protein was not a constraint
  +               ADPLSEFP  /1000      * vDMIntake(k,s,c)
  +        sum(x, ADPLSMaintAdv(c,s)/1000   * vNumberCows(c)          )
  +        sum(q, ADPLSMilk    * vMilkProd(k,c,s,q)        )
...
*  - sum(s, Sum(c, vNumberCows(k,c) * FertManureAdv(c,s,e) ))
  - 1000 * vFertReqt(k,e)                         =l= 0;                                  # was less than
...
eWaterBalance(k,s)..
  ...
  - vWaterReqt(k,s)                                        =l= 0;                      # was less than
...
*  +               Sum(c, vNumberCows(k,c)      * LCCAdv(s,c)  )
  + FixedLabour
  - vHireLab(k,s)                                     =l= OwnLab;                        # was less than
...
eCashLimit(k,s)..
$ontext
* The transfer system with an overdraft limit
  ...
  -  vOverdraft(k,s)        + vOverdraft(k,s-1)$(ord(s) > 1)*ODInt
                                       =l= (ord(s) = 1)*OpCash;
$offtext
*Basic transfer system for cash
  vCashTransfer(k,s)
  + vTCCowCosts(k,s)
  + vTCForageEstab(k,s)     + vTCFertPurch(k,s) + vTCWaterPurch(k,s)
  + vTCMakingConserved(k,s) + vTCFeedingConserved(k,s)
  + vTCSupplPurch(k,s)
  + vTCHiredLabour(k,s)     + vTCFixedCosts(k,s)
                                       =l= vTRMilkSales(k,s) + vTRCullSales(k,s) + vTRLeaseOut(k,s);
...
*Water
  + Sum(s, vForageArea(k,f) * WI(s,f)    )  * WaterPrice(k)                               #nb WaterCostDynamic not in here
                                                         =e= vTCGrowingAndFeeding(k,f);   #Could maybe be deleted, not part of analysis now, just for checking
